...
class FlymasterPointCreate(BaseModel):
    device_id: int
    date_time: datetime
    lat: float
    lon: float
...
class FlymasterBatchCreate(BaseModel):
    device_serial: str
    sha256key: str
    points: List[FlymasterPointCreate]

    class Config:
        json_schema_extra = {
            "example": {
                "device_serial": "FM123456",
                "sha256key": "a1b2c3d4e5f6...",
                "points": [
                    {
                        "device_id": 123456,
                        "date_time": "2025-06-05T13:13:56+03:00",
                        "lat": 45.5231,
                        "lon": -122.6765,
                        "gps_alt": 1200.5,
                        "heading": 180.0,
...
class FlymasterResponse(BaseModel):
    id: int
    device_id: int
    date_time: datetime
    lat: float
...
class FlymasterBatchResponse(BaseModel):
    device_serial: str
    points_added: int
    points_skipped: int
    message: str
...
class FlymasterFileUploadResponse(BaseModel):
    device_serial: str
    sha256key: str
    uploaded_at: datetime  # First timestamp from the file
    points_added: int
    points_skipped: int
    message: str
    success: bool = True
